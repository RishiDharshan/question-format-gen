...
async def run():
    config_path = 'content_meta/statistics_sample_10.json'
    
    # Load config to get db_file and outputs_dir
    with open(config_path, 'r') as f:
        data = json.load(f)
    
    db_file = data.get("db_file", "question_store/statistics_sample.sqlite3")
    ...
    output_filename = f"statistics_sample_10_output_{int(asyncio.get_event_loop().time())}.docx"
