...
async def run():
    config_path = 'content_meta/statistics_req1.json'
    
    # Load config to get db_file and outputs_dir
    with open(config_path, 'r') as f:
        data = json.load(f)
    
    db_file = data.get("db_file", "question_store/statistics.sqlite3")
    ...
    output_filename = f"statistics_req1_output_{int(asyncio.get_event_loop().time())}.docx"
